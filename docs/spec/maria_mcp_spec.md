...
> 분석결과 저장(`llm_analysis_result`) / Notification 관련 tool은 향후 별도 정의 (§10.2).
...
| `MARIA_POOL_MAX` | X | `10` | asyncmy 풀 최대 |
...
| tool | 분류 | 섹션 |
|------|------|------|
| `get_device_info` | 장비 단건 조회 | §4.1 |
| `get_alarm_detail` | 알람 단건 조회 | §4.2 |
| `get_active_alarms` | 미종료 알람 목록 | §4.3 |
| `get_alarms_by_time` | 시간 범위 알람 | §4.4 |
| `get_device_alarms` | 장비별 최근 알람 | §4.5 |
| `list_nearby_devices` | 같은 구역 장비 목록 | §10.1.1 |
| `list_devices_by_type` | 카테고리/유형별 장비 | §10.1.2 |
| `list_devices_by_ups` | UPS 연결 장비 목록 | §10.1.3 |
| `get_alarm_history` | 알람 이력 조회 | §10.1.4 |
| `get_active_alarm_summary` | 알람 카운트 요약 | §10.1.5 |
| `get_nearby_alarms` | 같은 구역 활성 알람 | §10.1.6 |
| `get_checkpoint_alarms` | 동일 체크포인트 알람 | §10.1.7 |
| `search_devices` | 다중 조건 장비 검색 | §10.1.8 |
| `get_alarm_history_by_policy` | 알람 정책별 이력 | §10.1.9 |
| `get_active_alarms_for_devices` | 다중 장비 활성 알람 | §10.1.10 |
...
### 4.4 `get_alarms_by_time`

지정한 시간 범위 내 발생한 알람 조회.
...
| `limit` | int | X | 50 | 최대 200 |

**SQL 대상**: `fm_fault_alarm_cur`
**필터**: `log_date >= start_time AND log_date < end_time`
**정렬**: `log_date DESC`

**응답**: `AlarmSummary` list (4.3과 동일 스키마)

---

### 4.5 `get_device_alarms`

특정 장비의 최근 N시간 알람 (RCA 컨텍스트용).
...
### 10.1 RCA 확장 도구 10개 (구현 완료)

Gemma4 기반 장애 원인 분석(RCA) 워크플로우를 지원하기 위해 논의·확정된 신규 tool.
`mcp_common_spec.md §7` tool 설계 원칙과 Gemma4 혼동 방지 docstring 원칙을 따른다.

> **Gemma4 docstring 원칙**: 유사 tool 간 혼동을 막기 위해 핵심 구분 문구를 docstring 첫 줄에 명시.
>
> | tool | 분류 | docstring 핵심 구분 문구 |
> |------|------|------------------------|
> | `get_device_info` (기존) | 장비 | "**단건** 장비 핵심 정보" |
> | `get_device_alarms` (기존) | 알람 | "최근 N시간 내 **현재 진행 중** 알람 (cur 테이블)" |
> | `get_active_alarms` (기존) | 알람 | "**전체** 데이터센터 미종료 알람 목록" |
> | `list_nearby_devices` (신규) | 장비 | "같은 구역 **장비 목록** (알람 아님)" |
> | `list_devices_by_type` (신규) | 장비 | "동일 카테고리/유형 **장비 목록**" |
> | `list_devices_by_ups` (신규) | 장비 | "특정 UPS에 **연결된 장비 목록**" |
> | `get_alarm_history` (신규) | 알람 | "과거 N일 **완료된** 알람 이력 (his 테이블)" |
> | `get_nearby_alarms` (신규) | 알람 | "같은 구역 다른 장비 **활성 알람**" |
> | `get_checkpoint_alarms` (신규) | 알람 | "**동일 모니터링 항목** 전 장비 활성 알람" |
> | `get_active_alarm_summary` (신규) | 알람 | "심각도별 **카운트 요약** (상세 목록 아님)" |
> | `search_devices` (신규) | 장비 | "device_id 불명 시 **다중 조건 부분 문자열 검색**" |
> | `get_alarm_history_by_policy` (신규) | 알람 | "알람 정책명 기준 **전 장비** 완료 이력 (his 테이블)" |
> | `get_active_alarms_for_devices` (신규) | 알람 | "**다중 장비** 활성 알람 일괄 조회 (IN 쿼리, N+1 방지)" |

---

#### 10.1.1 `list_nearby_devices`
...
| `data_center_id` | int | O | - | |
| `device_id` | int | O | - | |
| `days` | int | X | 30 | 최대 90 (파티션 보존 기간 이내) |
| `limit` | int | X | 20 | 최대 100 |
...
#### 10.1.5 `get_active_alarm_summary`
...
**응답 (AlarmSummary 대신 별도 모델 `AlarmSummaryStats`)**
...
**파라미터 선택 기준**:
- `location`: 층·구역·방 이름 텍스트 LIKE 검색. 예) `'3층'`, `'A구역'`, `'서버실'`. 사용자가 위치를 말로 표현하면 이 파라미터 사용.
- `floor_id` / `zone_id`: DB 내부 정수 ID (층 번호가 아님). 정확한 ID 값을 알고 있을 때만 사용.

**Gemma4 사용 시점**: "`device_id` 불명 시, 이름·위치·유형 등으로 장비를 먼저 찾아 `device_id` 획득"
...
#### 10.1.9 `get_alarm_history_by_policy`
...
#### 10.1.10 `get_active_alarms_for_devices`
