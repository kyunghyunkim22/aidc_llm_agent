...
_HOURS_MAX = 720        # 30일
_DAYS_MAX = 90          # his 테이블 파티션 보존 기간
_SEARCH_LIMIT_MAX = 100
_HISTORY_POLICY_LIMIT_MAX = 200
...
async def get_alarm_detail(data_center_id: int, alarm_id: int) -> AlarmDetail | None:
    with log_tool_call("get_alarm_detail", data_center_id=data_center_id, alarm_id=alarm_id) as meta:
        row = await db.fetch_one(
            "select_alarm_detail",
...
async def get_alarms_by_time(
    ...
    limit = _clamp(limit, 1, _ALARM_LIMIT_MAX)
    with log_tool_call(
        "get_alarms_by_time",
...
async def get_alarm_history(
    ...
    """과거 N일 완료된 알람 이력 (his 테이블) — 재발 패턴 파악용. days 최대 90."""
...
async def get_alarm_history_by_policy(
    ...
    """
    days = _clamp(days, 1, _DAYS_MAX)
    limit = _clamp(limit, 1, _HISTORY_POLICY_LIMIT_MAX)
    with log_tool_call(
...
async def search_devices(
    ...
    limit = _clamp(limit, 1, _SEARCH_LIMIT_MAX)
